...
func (app *application) GetPaymentIntent(w http.ResponseWriter, r *http.Request) {
	...
	pi, msg, err := crd.Charge(payload.Currency, amount)
	if err != nil {
		okay = false
	}
...
func (app *application) CreateCustomerAndSubscribeToPlan(w http.ResponseWriter, r *http.Request) {
	...
	stripeCustomer, msg, err := crd.CreateCustomer(data.PaymentMethod, data.Email)
	if err != nil {
		app.errorLog.Println(err)
		okay = false
		txnMsg = msg
	...
	if okay {
		subscription, err = crd.SubscribeToPlan(
			stripeCustomer,
			data.Plan,
			data.Email,
			data.LastFour,
...
func (app *application) VirtualTerminalPaymentSucceeded(w http.ResponseWriter, r *http.Request) {
	...
	pi, err := crd.RetrievePaymentIntent(txnData.PaymentIntent)
	if err != nil {
		app.badRequest(w, r, err)
		return
	}

	pm, err := crd.GetPaymentMethod(txnData.PaymentMethod)
	if err != nil {
		app.badRequest(w, r, err)
		return
	}
